...
class SocketService {
  ...
  // final countController = StreamController<Map>();
  static final messageController = StreamController<List<Message>>();

  static Stream<List<Message>> get messageStream => messageController.stream;

  // static const String liveClassEndPoint =
  //     "https://planner-live-classes.toppersnotes.com";
  //
  static const String liveClassEndPoint =
      "https://virtual-classes.toppersnotes.com/class";
  ...
  static void connectToServer() {
    try {
      ...
      socket.onConnect((handler) {
        ...
        socket.on('class_started', (data) => print("class_started: $data"));
        socket.on('class_finished', (data) => print("class_finished: $data"));
        socket.on('count', (data) => print(data));
        // socket.on('new_comment', (data) => messageController.sink.add(data));
        socket.on('old_comment', (data) => messageController.sink.add(data));
